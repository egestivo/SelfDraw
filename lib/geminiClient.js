...
- **ACCIÓN OBLIGATORIA:** Usa la etiqueta [ACCION: MOSTRAR_TEST] seguida del ID del test.
- **TESTS DISPONIBLES:**
    - Para ansiedad: [ACCION: MOSTRAR_TEST] [TEST: gad-7]
    - Para depresión: [ACCION: MOSTRAR_TEST] [TEST: phq-9]
    - Para motivación: [ACCION: MOSTRAR_TEST] [TEST: sms-6]
- **IMPORTANTE:** NO generes JSON. Solo usa el ID del test (gad-7, phq-9 o sms-6).
...
- **Paso Final:** Vuelve a aplicar el test correspondiente usando [ACCION: MOSTRAR_TEST] [TEST: id] para medir si hubo mejoría.
...
- **NUNCA** respondas con JSON crudo fuera de las etiquetas \`[TEST: ...]\` o \`[GUIA: ...]\`.
- Para tests, usa SOLO el ID: [ACCION: MOSTRAR_TEST] [TEST: gad-7]
- Cuando decidas el estado del usuario para cambiar la interfaz, incluye al final de tu respuesta:
...
async function getChatResponse(history, message) {
    try {
        ...
        if (turnCount < 2) {
            ...
            phaseInstruction = "ESTÁS EN FASE 1 (MEDICIÓN). DEBES enviar el test ahora. Usa [ACCION: MOSTRAR_TEST] [TEST: gad-7] para ansiedad, [TEST: phq-9] para depresión o [TEST: sms-6] para motivación. NO generes JSON.";
        } else {
